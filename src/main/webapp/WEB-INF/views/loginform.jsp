...
        .login-form { width: 100%; } /* 너비를 100%로 조정하여 정렬을 맞춥니다. */
...
        .input-group input { width: 100%; box-sizing: border-box; /* 패딩과 보더를 너비에 포함시킵니다. */ padding: 13px 14px 13px 44px; border: 1.5px solid #d6e0ea; border-radius: 8px; font-size: 16px; color: #2d3a4b; background: #f9fbfd; outline: none; transition: border-color 0.25s, box-shadow 0.25s; font-family: inherit; }
...
        .alert { width: 100%; box-sizing: border-box; /* 너비를 100%로 맞춥니다. */ margin: 0 0 18px 0; padding: 12px 18px; border-radius: 8px; background: #ffeaea; color: #e74c3c; font-size: 15px; text-align: center; }
        @media (max-width: 600px) { .login-container { width: 100%; padding: 24px 6px; border-radius: 0; box-shadow: none; } .book-bg { width: 100vw; } }
    </style>
    <script>
        window.onload = function() {
            // [수정 1] 로그인 실패 시 URL 파라미터 'error'를 확인하여 아이디 입력란에 포커스
...
<body>
    ...
    <div class="login-container">
        ...
        <c:if test="${param.error != null}">
            <div class="alert">아이디 또는 비밀번호가 잘못되었습니다.</div>
        </c:if>
